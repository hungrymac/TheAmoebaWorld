...
export function ActivityFeed({ items }: ActivityFeedProps) {
  return (
    <Card className="border-muted/60 shadow-sm">
      ...
      <CardContent className="space-y-0 divide-y rounded-b-xl border-t bg-muted/10 px-0 pb-0">
        {items.map((item) => (
          <div
            key={item.id}
            className="flex gap-3 px-6 py-4 first:pt-5 last:pb-5 hover:bg-muted/30"
          >
